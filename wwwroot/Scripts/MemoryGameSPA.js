...
function startTimer(){
  function loop(){
    const _1=null;
    return Delay(() => Bind(Sleep(1000), () => {
      gameStateVar().Set(tickTimer(gameStateVar().Get()));
      return!gameStateVar().Get().IsGameOver?Bind(loop(), () => Return(null)):Zero();
    }));
  }
  Start(loop(), null);
}
function tickTimer(state){
  if(state.IsGameOver)return state;
    ...
    const players=mapi((_1, _2) => _1===state.CurrentPlayer&&_2.TimeLeft>0?New_1(_2.Name, _2.Score, _2.TimeLeft-1, _2.Lives):_2, state.Players);
    const currPlayer=players.get_Item(state.CurrentPlayer);
    return forAll((p) => p.TimeLeft<=0, players)?New(state.Cards, state.Flipped, players, state.CurrentPlayer, true, state.Streak):currPlayer.TimeLeft<=0?New(state.Cards, state.Flipped, players, (state.CurrentPlayer+1)%players.Length, state.IsGameOver, state.Streak):New(state.Cards, state.Flipped, players, state.CurrentPlayer, state.IsGameOver, state.Streak);
  }
}
function gameStateVar(){
  return _c.gameStateVar;
}
function initGame(size, names){
  return New(createDeck(size), FSharpList.Empty, createPlayers(names), 0, false, 0);
}
function playerNamesVar(){
  return _c.playerNamesVar;
}
function nameInputView(){
  return _c.nameInputView;
}
function playerStatsView(state){
  return Doc.Element("div", [], mapi((_1, _2) => Doc.Element("div", [Attr.Create("style", _1===state.CurrentPlayer?"font-weight:bold; color:green;":"color:black;")], [Doc.TextNode(String(_2.Name)+" - Pont: "+String(_2.Score)+", Id\u0151: "+String(_2.TimeLeft)+"s, Életek: "+String(_2.Lives))]), state.Players));
}
function cardsView(state){
  return Doc.Element("div", [Attr.Create("class", "cards")], mapi(cardView, state.Cards));
}
function resetGame(e){
  startTimer(e);
  gameStateVar().Set(initGame(8, ofArray(["Játékos1", "Játékos2"])));
}
function createDeck(size){
  return mapi((_1, _2) => New_2(_1, _2, false, false), sortBy(() => random().Next_2(), collect((emoji) => ofArray([emoji, emoji]), ofSeq(take(size, emojis())))));
}
function createPlayers(names){
  return map((name) => New_1(name, 0, 60, 5), names);
}
function cardView(index, card){
  return Doc.Element("div", [Attr.Create("class", concat(" ", ofSeq(delay(() => append_1(["card"], delay(() => append_1(card.IsFlipped||card.IsMatched?["flipped"]:[], delay(() => card.IsMatched?["matched"]:[])))))))), Handler("click", () =>(e) => onCardClick(index, e))], [Doc.Element("div", [Attr.Create("class", "card-inner")], [Doc.Element("div", [Attr.Create("class", "card-front")], [Doc.TextNode("\u2753")]), Doc.Element("div", [Attr.Create("class", "card-back")], [Doc.TextNode(card.Value)])])]);
}
function emojis(){
  return _c.emojis;
}
...
function onCardClick(index){
  const state=gameStateVar().Get();
  if(!state.IsGameOver&&state.Flipped.Length<2){
    const state1=flipCard(state, index);
    gameStateVar().Set(state1);
    if(state1.Flipped.Length===2){
      const _1=null;
      Start(Delay(() => Bind(Sleep(1000), () => {
        const newState=checkMatch(gameStateVar().Get());
        const m=state1.Flipped;
        const finalState=!(m.$==1&&(m.$1.$==1&&(m.$1.$1.$==0&&state1.Cards.get_Item(m.$0).Value==state1.Cards.get_Item(m.$1.$0).Value)))?nextPlayer(newState):newState;
        gameStateVar().Set(finalState);
        return Zero();
      })), null);
    }
    else void 0;
...
function flipCard(state, index){
  return contains(index, state.Flipped)||state.Cards.get_Item(index).IsMatched?state:New(mapi((_1, _2) => _1===index?New_2(_2.Id, _2.Value, true, _2.IsMatched):_2, state.Cards), FSharpList.Cons(index, state.Flipped), state.Players, state.CurrentPlayer, state.IsGameOver, state.Streak);
}
function checkMatch(state){
  const m=state.Flipped;
  if(m.$==1){
    if(m.$1.$==1){
      if(m.$1.$1.$==0){
        const i1=m.$0;
        const i2=m.$1.$0;
        if(state.Cards.get_Item(i1).Value==state.Cards.get_Item(i2).Value){
          const cards=mapi((_1, _2) => _1===i1||_1===i2?New_2(_2.Id, _2.Value, _2.IsFlipped, true):_2, state.Cards);
          const players=mapi((_1, _2) => _1===state.CurrentPlayer?New_1(_2.Name, _2.Score+1, _2.TimeLeft, _2.Lives):_2, state.Players);
          const isGameOver=allMatched(cards);
          const newStreak=state.Streak+1;
          return New(newStreak>=3?shuffleUnmatched(cards):cards, FSharpList.Empty, players, state.CurrentPlayer, isGameOver, newStreak%3);
        }
        else {
          const cards_1=mapi((_1, _2) => _1===i1||_1===i2?New_2(_2.Id, _2.Value, false, _2.IsMatched):_2, state.Cards);
          const players_1=mapi((_1, _2) => _1===state.CurrentPlayer?New_1(_2.Name, _2.Score, _2.TimeLeft, _2.Lives-1):_2, state.Players);
          return New(cards_1, FSharpList.Empty, players_1, state.CurrentPlayer, players_1.get_Item(state.CurrentPlayer).Lives<=0||allMatched(cards_1), 0);
        }
      }
      else return state;
    }
...
function nextPlayer(state){
  return New(state.Cards, FSharpList.Empty, state.Players, (state.CurrentPlayer+1)%state.Players.Length, state.IsGameOver, state.Streak);
}
function shuffleUnmatched(cards){
  const shuffled=sortBy(() => random().Next_2(), filter((c) =>!c.IsMatched, cards));
  return mapi((_1, _2) => New_2(_1, _2.Value, _2.IsFlipped, _2.IsMatched), append(filter((c) => c.IsMatched, cards), shuffled));
}
function allMatched(cards){
  return forAll((c) => c.IsMatched, cards);
}
...
let _c=Lazy((_i) => class $StartupCode_Client {
  ...
  static gameStateVar;
  static playerNamesVar;
  static random;
  static emojis;
  static {
    this.emojis=ofArray(["\ud83d\udc36", "\ud83d\udc31", "\ud83e\udd8a", "\ud83d\udc3c", "\ud83d\udc35", "\ud83d\udc38", "\ud83d\udc19", "\ud83e\udd84"]);
    this.random=new Random();
    this.playerNamesVar=_c_1.Create_1(null);
    this.gameStateVar=_c_1.Create_1(initGame(8, ofArray(["Játékos1", "Játékos2"])));
    const name1=_c_1.Create_1("");
    const name2=_c_1.Create_1("");
    let _1=Doc.Element("div", [Attr.Create("style", "margin: 2rem;")], [Doc.Element("h3", [], [Doc.TextNode("Add meg a játékosok neveit:")]), Doc.Element("div", [], [Doc.Element("label", [], [Doc.TextNode("Játékos 1: ")]), Doc.Input([], name1)]), Doc.Element("div", [Attr.Create("style", "margin-top: 1rem;")], [Doc.Element("label", [], [Doc.TextNode("Játékos 2: ")]), Doc.Input([], name2)]), Doc.Element("button", [Attr.Create("style", "margin-top: 1rem;"), Handler("click", () => {
      const names=map((n) => n==""?"Névtelen":n, ofArray([name1.Get(), name2.Get()]));
      playerNamesVar().Set(Some(names));
      gameStateVar().Set(initGame(8, names));
      return(e) => {
        startTimer(e);
      };
    })], [Doc.TextNode("\u25b6\ufe0f Játék indítása")])]);
    this.nameInputView=_1;
    this.mainView=Doc.BindView((a) => a==null?nameInputView():Doc.Element("div", [Attr.Create("class", "game-board")], [Doc.BindView(playerStatsView, gameStateVar().View), Doc.BindView(cardsView, gameStateVar().View), Doc.BindView((state) => {
      if(state.IsGameOver){
        const maxScore=maxBy((p) => p.Score, state.Players).Score;
        exists((p) => p.Lives<=0, state.Players);
        const winners=filter((p) => p.Score===maxScore, state.Players);
        const winnerText=winners.Length===1?"\ud83c\udfc6 A gy\u0151ztes: "+head(winners).Name+"!":"\ud83e\udd1d Döntetlen! Gy\u0151ztesek: "+concat(", ", map((p) => p.Name, winners));
        return Doc.Element("div", [Attr.Create("style", "margin-top:1rem;")], [Doc.Element("div", [Attr.Create("style", "color:red; font-weight:bold; margin-bottom:0.5rem;")], [Doc.TextNode("Véget ért a játék!")]), Doc.Element("div", [Attr.Create("style", "margin-bottom:0.5rem; color:blue; font-weight:bold;")], [Doc.TextNode(winnerText)]), Doc.Element("button", [Attr.Create("class", "restart-button"), Handler("click", () => resetGame)], [Doc.TextNode("\ud83d\udd01 Új játék indítása")])]);
      }
      else return Doc.Empty;
    }, gameStateVar().View)]), playerNamesVar().View);
  }
});
...
function New(Cards, Flipped, Players, CurrentPlayer, IsGameOver, Streak){
  return{
    Cards:Cards, 
    Flipped:Flipped, 
    Players:Players, 
...
function New_1(Name, Score, TimeLeft, Lives){
  return{
    Name:Name, 
    Score:Score, 
    TimeLeft:TimeLeft, 
...
function New_2(Id, Value_1, IsFlipped, IsMatched){
  return{
    Id:Id, 
    Value:Value_1, 
    IsFlipped:IsFlipped, 
...
function New_5(DynElem, DynFlags, DynNodes, OnAfterRender){
  const _1={
    DynElem:DynElem, 
    DynFlags:DynFlags, 
    DynNodes:DynNodes
